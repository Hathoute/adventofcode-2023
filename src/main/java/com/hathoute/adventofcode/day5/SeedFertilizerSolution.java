...
public class SeedFertilizerSolution {
    ...
    static class SeedFertilizer implements AdventOfCodePuzzle {
        ...
        private long compute(Map<String, Tuple2<String, List<RangeMapEntry>>> rangeMap, String init, long value) {
            var currentKey = init;
            ...
            while (rangeMap.containsKey(currentKey)) {
                ...
                currentVal = ranges.right().stream()
        ...
        private Map<String, Tuple2<String, List<RangeMapEntry>>> parseMaps(List<String> lines) {
        ...
        private static List<Long> parseInitializer(String firstLine) {
            var separatorIndex = firstLine.indexOf(':');
            return parseLongs(firstLine.substring(separatorIndex + 1), " ").toList();
        }

        private static Tuple3<String, String, List<RangeMapEntry>> parseMap(List<String> subList) {
